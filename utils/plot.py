...
try:
    with open(data_path, "r") as f:
        ...
        for row in reader:
            ...
            ntl_fft_vals.append(float(row["ntl_fft"]))
            ntl_fft_norm.append(float(row["ntl_fft_over_nlogn"]))
...
ax1.plot(n_vals, ntl_fft_vals, marker='x', label='NTL FFT')
...
ax2.plot(n_vals, ntl_fft_norm, marker='x', color='red', label='NTL FFT / ($n \log n$)')
