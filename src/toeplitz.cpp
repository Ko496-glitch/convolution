...
#include <random>          // FIX: random_device, mt19937, distributions

#include <fftw3.h>         // FIX: fftw_complex, fftw_malloc, fftw_plan
...
void generate_toeplitz_data(std::vector<double>& vec, long n) {
    ...
    for (long i = 0; i < 2 * n - 1; ++i) vec[i] = dis(gen);
}
...
void fftw_toeplitz_convolve(const double* input, const double* mat, double* output, int n) {
    ...
    fftw_complex *output_fft = (fftw_complex*) fftw_malloc(sizeof(fftw_complex) * m);
    ...
    for (int i = 0; i < m; ++i) {
        output_fft[i][0] = in_fft[i][0] * mat_fft[i][0] - in_fft[i][1] * mat_fft[i][1]; // FIX
        output_fft[i][1] = in_fft[i][0] * mat_fft[i][1] + in_fft[i][1] * mat_fft[i][0]; // FIX
    }
